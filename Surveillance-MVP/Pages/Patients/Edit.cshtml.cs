...
namespace Surveillance_MVP.Pages.Patients
{
    ...
    public class EditModel : PageModel
    {
        ...
        public EditModel(Surveillance_MVP.Data.ApplicationDbContext context, IGeocodingService geocoder, IPatientCustomFieldService customFieldService, IAuditService auditService, IPatientIdGeneratorService patientIdGenerator)
        {
            _context = context;
            _geocoder = geocoder;
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                ...
                // Generate FriendlyId if it's null or empty
        ...
        private async Task LogPatientChangesAsync(Patient oldPatient, Patient newPatient, string? userId, string? ipAddress)
        {
            ...
            if (oldPatient.EthnicityId != newPatient.EthnicityId)
            {
                var oldEth = oldPatient.EthnicityId.HasValue ? (await _context.Ethnicities.FindAsync(oldPatient.EthnicityId))?.Name : null;
                var newEth = newPatient.EthnicityId.HasValue ? (await _context.Ethnicities.FindAsync(newPatient.EthnicityId))?.Name : null;
                await _auditService.LogChangeAsync("Patient", patientIdString, "Ethnicity", oldEth, newEth, userId, ipAddress);
            }
