...
public interface IOutbreakService
{
    ...
    Task<bool> UpdateConfirmationStatusAsync(int outbreakId, string statusName, string notes);
    
    // Parent-Child outbreak management
    Task<Outbreak> CreateChildOutbreakAsync(int parentId, Outbreak childOutbreak, string userId);
    Task<List<Outbreak>> GetChildOutbreaksAsync(int parentId);
    Task<List<Outbreak>> GetAllDescendantOutbreaksAsync(int parentId);
    Task<OutbreakStatistics> GetAggregatedStatisticsAsync(int parentId);
    ...
    Task<bool> AddTimelineEventAsync(int outbreakId, string title, string? description, DateTime eventDate, TimelineEventType eventType, string userId);
    ...
    Task<OutbreakTaskSummary> GetTaskStatusSummaryRecursiveAsync(int outbreakId);
    Task<Dictionary<string, List<CaseTask>>> GetTasksByCaseRecursiveAsync(int outbreakId);
    Task<List<CaseTask>> GetOverdueTasksRecursiveAsync(int outbreakId);
}
