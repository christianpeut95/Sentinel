...
public class SuperviseInterviewsModel : PageModel
{
    ...
    // Unassigned section pagination/filters
    [BindProperty(SupportsGet = true)]
    public int UnassignedPage { get; set; } = 1;

    [BindProperty(SupportsGet = true)]
    public int UnassignedPageSize { get; set; } = 20;

    [BindProperty(SupportsGet = true)]
    public string? FilterUnassignedPriority { get; set; }

    [BindProperty(SupportsGet = true)]
    public string? FilterUnassignedSearch { get; set; }

    [BindProperty(SupportsGet = true)]
    public string? FilterUnassignedLanguage { get; set; }

    // Escalated section pagination/filters
    [BindProperty(SupportsGet = true)]
    public int EscalatedPage { get; set; } = 1;

    [BindProperty(SupportsGet = true)]
    public int EscalatedPageSize { get; set; } = 10;

    [BindProperty(SupportsGet = true)]
    public string? FilterEscalatedSearch { get; set; }

    // Paginated data for unassigned/escalated sections
    public List<CaseTask> UnassignedTasks { get; set; } = new();
    public int UnassignedTotalTasks { get; set; }
    public int UnassignedTotalPages => (int)Math.Ceiling((double)UnassignedTotalTasks / UnassignedPageSize);

    public List<CaseTask> EscalatedTasks { get; set; } = new();
    public int EscalatedTotalTasks { get; set; }
    public int EscalatedTotalPages => (int)Math.Ceiling((double)EscalatedTotalTasks / EscalatedPageSize);
    ...
    public async Task OnGetAsync()
    {
        try
        {
            _logger.LogInformation("SuperviseInterviews OnGetAsync started. Page: {Page}, PageSize: {PageSize}, FilterWorker: {FilterWorker}, FilterPriority: {FilterPriority}, SearchTerm: {SearchTerm}", 
                PageNumber, PageSize, FilterWorker, FilterPriority, SearchTerm);
            
            // Get summary data (always needed)
            DashboardData = await _assignmentService.GetSupervisorDashboardSummaryAsync();
            _logger.LogInformation("Dashboard summary loaded. Unassigned: {Unassigned}, Escalated: {Escalated}, ActiveWorkers: {ActiveWorkers}", 
                DashboardData?.UnassignedTaskCount ?? 0, DashboardData?.EscalatedTaskCount ?? 0, DashboardData?.ActiveWorkerCount ?? 0);
            _logger.LogInformation("Dashboard UnassignedTasks list: {Count} items", DashboardData?.UnassignedTasks?.Count ?? 0);
            _logger.LogInformation("Dashboard EscalatedTasks list: {Count} items", DashboardData?.EscalatedTasks?.Count ?? 0);
            _logger.LogInformation("Dashboard WorkerStats list: {Count} items", DashboardData?.WorkerStats?.Count ?? 0);
            ...
            _logger.LogInformation("Assigned tasks loaded. CurrentlyAssignedTasks: {Count}, TotalTasks: {Total}", 
                CurrentlyAssignedTasks?.Count ?? 0, TotalTasks);
            
            // Get available workers for filters and assignment
            AvailableWorkers = await _assignmentService.GetAllInterviewWorkersAsync();
            _logger.LogInformation("Available workers loaded: {Count}", AvailableWorkers?.Count ?? 0);

            // Get paginated unassigned tasks
            var (unassignedTasks, unassignedTotal) = await _assignmentService.GetUnassignedInterviewTasksPaginatedAsync(
                pageNumber: UnassignedPage,
                pageSize: UnassignedPageSize,
                priority: FilterUnassignedPriority,
                searchTerm: FilterUnassignedSearch,
                language: FilterUnassignedLanguage
            );
            UnassignedTasks = unassignedTasks;
            UnassignedTotalTasks = unassignedTotal;

            // Get paginated escalated tasks
            var (escalatedTasks, escalatedTotal) = await _assignmentService.GetEscalatedInterviewTasksPaginatedAsync(
                pageNumber: EscalatedPage,
                pageSize: EscalatedPageSize,
                searchTerm: FilterEscalatedSearch
            );
            EscalatedTasks = escalatedTasks;
            EscalatedTotalTasks = escalatedTotal;
