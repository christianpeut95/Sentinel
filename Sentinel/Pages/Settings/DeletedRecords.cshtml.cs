...
namespace Sentinel.Pages.Settings
{
    ...
    public class DeletedRecordsModel : PageModel
    {
        ...
        public List<ExposureEvent> DeletedExposureEvents { get; set; } = new();
        ...
        public async Task OnGetAsync()
        {
            ...
            if (string.IsNullOrEmpty(Filter) || Filter == "exposures")
            {
                DeletedExposureEvents = await _context.OnlyDeleted<ExposureEvent>()
                    .Include(ee => ee.ExposedCase)
                        .ThenInclude(c => c.Patient)
                    .Include(ee => ee.Event)
                    .Include(ee => ee.Location)
                    .Include(ee => ee.SourceCase)
                    .OrderByDescending(ee => ee.DeletedAt)
                    .Take(100)
                    .ToListAsync();
            }
        }
        ...
        public async Task<IActionResult> OnPostRestoreExposureAsync(Guid id)
        {
            var exposure = await _context.OnlyDeleted<ExposureEvent>()
                .Include(ee => ee.ExposedCase)
                .FirstOrDefaultAsync(ee => ee.Id == id);
            
            if (exposure == null)
            {
                TempData["ErrorMessage"] = "Exposure event not found.";
                return RedirectToPage();
            }

            await _context.RestoreAsync(exposure);
            TempData["SuccessMessage"] = $"Exposure event for case '{exposure.ExposedCase?.FriendlyId}' has been restored.";
            return RedirectToPage(new { Filter = "exposures" });
        }

        public async Task<IActionResult> OnPostPermanentDeleteExposureAsync(Guid id)
        {
            var exposure = await _context.OnlyDeleted<ExposureEvent>().FirstOrDefaultAsync(ee => ee.Id == id);
            if (exposure == null)
            {
                TempData["ErrorMessage"] = "Exposure event not found.";
                return RedirectToPage();
            ...
            await _context.SaveChangesAsync();
            TempData["SuccessMessage"] = "Exposure event has been permanently deleted.";
            return RedirectToPage(new { Filter = "exposures" });
        }
    }
}
