...
namespace Sentinel.Pages.Settings.Surveys
{
    ...
    public class DesignSurveyModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(Guid? id, [FromBody] SaveSurveyRequest request)
        {
            ...
            var isFirstSave = string.IsNullOrWhiteSpace(oldJson);
            ...
            // If this is the first save (no previous definition), mark as active and published
            if (isFirstSave)
            {
                surveyTemplate.VersionStatus = SurveyVersionStatus.Active;
                surveyTemplate.PublishedAt = DateTime.UtcNow;
                surveyTemplate.PublishedBy = User.Identity?.Name;
                _logger.LogInformation(
                    "Survey template {TemplateId} first save - marking as Active",
                    surveyTemplate.Id);
            }
