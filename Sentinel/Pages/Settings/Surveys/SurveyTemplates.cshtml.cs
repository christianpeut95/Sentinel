...
namespace Sentinel.Pages.Settings.Surveys
{
    ...
    public class SurveyTemplatesModel : PageModel
    {
        ...
        public async Task OnGetAsync(string? category, bool? active, string? search)
        {
            ...
            // Get all surveys and filter to latest versions in memory
            // This avoids complex GroupBy SQL that might have issues with nullable GUIDs
            var allSurveys = await _context.SurveyTemplates
                .Include(st => st.ApplicableDiseases)
                    .ThenInclude(std => std.Disease)
                .ToListAsync();

            // Group by family and take latest version
            var latestVersions = allSurveys
                .GroupBy(st => st.ParentSurveyTemplateId ?? st.Id)
                .Select(g => g.OrderByDescending(st => st.Version).First())
                .ToList();

            _logger.LogInformation("Found {Count} latest survey versions out of {Total} total surveys",
                latestVersions.Count, allSurveys.Count);

            // Apply filters
            var query = latestVersions.AsQueryable();
